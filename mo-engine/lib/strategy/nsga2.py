...
class NSGA2LoopStrategy(EngineStrategy):
    """
    NSGA-II loop strategy integrated with SimLab.
    ...
    - Upon receiving the results (DONE status on the Generation), calculates the objectives,
        performs NSGA-II environmental selection with crowding distance tie-breaking,
    ...
    def __init__(self, experiment: dict, mongo):
        ...
        if self._is_synthetic:
            logger.info("[NSGA2] Synthetic mode enabled — skipping CSC/source-repo for simulations.")
...
# ------------------------------
    ...
    def event_simulation_done(self, sim_doc: dict):
        ...
        with self._lock:
            ...
            logger.info(
                "[NSGA-II] Simulation terminal (%s): %d/%d for generation %s",
    ...
    def event_generation_done(self, gen_doc: dict):
        ...
        Controls the algorithm flow: objective extraction, NSGA-II selection, next generation.
        """
    ...
    def _handle_generation_done(self, gen_oid: ObjectId):
        ...
        if self.mongo.experiment_repo.is_cancelled(str(self._exp_id)):
            logger.info("[NSGA-II] Experiment %s cancelled; stopping without finalizing.", self._exp_id)
            self.stop()
            return
...
# ------------------------------
    def _load_genome_cache_from_db(self) -> None:
        ...
        if entries:
            logger.info(
                "[NSGA-II] Genome cache loaded: %d registered, %d with objectives.",
                len(entries),
    ...
    def _resume_existing_generation(self, generations: list[Generation]) -> bool:
        ...
        if status in (EnumStatus.WAITING, EnumStatus.RUNNING):
            logger.info(
                "[NSGA-II] Resuming experiment %s from existing generation %s (index=%d, status=%s).",
                self._exp_id,
                self._generation_id,
        ...
        if status == EnumStatus.DONE:
            logger.info(
                "[NSGA-II] Replaying completed generation %s (index=%d) to continue the experiment.",
                self._generation_id,
                last_generation["index"],
            )
            self.mongo.experiment_repo.update(str(self._exp_id), {
        ...
        raise RuntimeError(
            f"Cannot resume NSGA-II from generation index={last_generation['index']} with status={status}."
        )
...
# ------------------------------
        ...
        def _run_generation_watcher():
            def _callback(gen_doc: dict):
                ...
                try:
                    ...
                    logger.exception("[NSGA-II] Generation watcher callback error.")

            logger.info("[NSGA-II] Starting Generation watcher (DONE or ERROR).")
        ...
        def _run_simulation_watcher():
            def _callback(sim_doc: dict):
                ...
                try:
                    ...
                    logger.exception("[NSGA-II] Simulation watcher callback error.")

            logger.info("[NSGA-II] Starting Simulation watcher (DONE or ERROR).")
            self.mongo.simulation_repo.watch_status_terminal(_callback)

        self._watch_thread = Thread(target=_run_generation_watcher, daemon=True, name="nsga2-gen-watcher")
        self._sim_watch_thread = Thread(target=_run_simulation_watcher, daemon=True, name="nsga2-sim-watcher")
    ...
    def _reconcile_current_generation(self) -> None:
        ...
        try:
            ...
            logger.exception("[NSGA-II] Reconcile: failed to read generation status.")
            return
        if gen and gen.get("status") in (EnumStatus.DONE, EnumStatus.ERROR):
            logger.warning(
                "[NSGA-II] Generation %s already %s at startup; processing (watcher race).",
        ...
        def _poll():
            while not self._stop_flag:
                ...
                try:
                    ...
                    if gen and gen.get("status") in (EnumStatus.DONE, EnumStatus.ERROR):
                        logger.warning(
                            "[NSGA-II] Generation %s %s detected by polling fallback.",
                            self._generation_id, gen.get("status")
                        )
                        with self._lock:
                            self._handle_generation_done(ObjectId(gen["_id"]))
                except Exception:
                    logger.exception("[NSGA-II] Polling fallback error.")

        Thread(target=_poll, daemon=True, name="nsga2-poll").start()
    ...
    def _upload_topology_async(
        ...
        def _do():
            try:
                ...
                logger.exception(
                    "[NSGA-II] Topology upload failed for gen=%s ind=%s", gen_index, ind_idx
                )
...
# ------------------------------
    def _generation_enqueue(self) -> None:
        ...
        # When every genome already had cached objectives there are no simulations
        # to wait for. Mark the generation DONE directly so the change-stream
        # watcher / reconcile advances the algorithm to the next generation.
        all_cached = sims_inserted == 0
        if all_cached:
            self.mongo.generation_repo.mark_done(gen_oid)
            logger.info(
                "[NSGA-II] Generation %d: all %d genomes have cached objectives; marked DONE.",
            ...
            Thread(
                ...
                args=(gen_oid,),
                daemon=True,
                name=f"nsga2-gen-done-{gen_index}",
            ).start()
        else:
            logger.info(
                "[NSGA-II] Generation %d enqueued with %d individuals (%d new simulations).",
    ...
    def _fire_generation_done(self, gen_oid: ObjectId) -> None:
        ...
        try:
            ...
            logger.exception("[NSGA-II] Direct generation-done trigger failed.")
...
# ---------------------------------------
    def _evolution(self) -> None:
        ...
        Environmental selection uses NSGA-II crowding distance: whole fronts are accepted
        in rank order; the last partial front is filled by descending crowding distance.
        """
        # ---------------- First PHASE P_1 ----------------
        if self._parents == []:
...
# ------------------------------
    def _convert_genome_to_sim_config(self,
        ...
        config: SimulationConfig = {
            "name": f"nsga2-g{gen_index}-{ind_idx}-{seed}",
...
# ---------------------------------------
    def _finalize_experiment(self,
        ...
        if pareto_front is not None:
            logger.info(f"[NSGA-II] Experiment {self._exp_id} completed.")
            self.mongo.experiment_repo.update(str(self._exp_id), {
            ...
            logger.error(f"[NSGA-II] Experiment {self._exp_id} finished with error.")
            self.mongo.experiment_repo.update(str(self._exp_id), {
