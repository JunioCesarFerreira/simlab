...
class RandomSearchStrategy(EngineStrategy):
        ...
        for i, genome in enumerate(population):
            ...
            if self._problem_adapter.is_analytical:
                ...
                try:
                    ...
                    logger.warning("[RandomSearch] genome_cache write failed for %s; continuing.", genome_hash)
                config_topo = self._convert_genome_to_sim_config(
                    genome=genome, gen_index=gen_index, ind_idx=i, seed=first_seed,
                )
                self._upload_topology_async(
    ...
    def _handle_generation_done(self, gen_oid: ObjectId):
        ...
        if self.mongo.experiment_repo.is_cancelled(str(self._exp_id)):
            logger.info("[RandomSearch] Experiment %s cancelled; stopping without finalizing.", self._exp_id)
            self.stop()
            return
