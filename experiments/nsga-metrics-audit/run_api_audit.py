...
def main():
    ...
    factory.individual_repo.find_grouped_by_experiment.return_value = {gen_id: [{"objectives": [1., 9.]}]}
    ...
    def call(names=("f1", "f2")):
        return get_hv_gd(str(exp_id), list(names), ["true", "true"], population="survivors", normalize=True, factory=factory)
    normal = call()
    reordered = call(("f2", "f1"))
    result = {"gd_original_order": normal["gd"], "gd_reordered_names": reordered["gd"]}
    factory.individual_repo.find_grouped_by_experiment.return_value = {gen_id: [{"objectives": [1e9, 1e9]}]}
    try:
    ...
    factory.individual_repo.find_grouped_by_experiment.return_value = {gen_id: [{"objectives": [0.5, 0.5]}]}
